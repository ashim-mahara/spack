...
from spack.compilers.environment import LibcDetector
...
def c_compiler_runs(compiler: spack.compiler.Compiler) -> bool:
    return LibcDetector(compiler).compiler_verbose_output() is not None
...
def _external_config_with_implicit_externals(configuration):
    ...
    for compiler in spack.compilers.all_compilers_from(configuration):
        libc = LibcDetector(compiler).default_libc()
        if libc:
...
class SpackSolverSetup:
            ...
            for dspec in spec.edges_to_dependencies():
                ...
                if spec.concrete:
                    ...
                    if dep.name == "gcc-runtime":
                        # FIXME (compiler as nodes): determine runtime compatibility
                        clauses.append(
                ...
                if not spec.concrete or expand_hashes:
                    # FIXME (compiler as nodes): add a depends_on(type="build")
    ...
    def setup(
        ...
        *,
        ...
        compiler_parser = CompilerParser(configuration=spack.config.CONFIG)
        ...
        # FIXME (compiler as nodes): this should not be needed anymore
        ...
        # FIXME (compiler as nodes): recover the information below
        # self.compiler_facts()
    ...
    def define_runtime_constraints(self):
        ...
        for compiler in self.possible_compilers:
            ...
            current_libc = LibcDetector(compiler).default_libc()
...
# FIXME (compiler as nodes): a class seems an overkill now for compilers,
class CompilerParser:
    ...
    def __init__(self, configuration) -> None:
        ...
        for c in spack.compilers.all_compilers_from(configuration):
            ...
            if using_libc_compatibility() and not LibcDetector(c).default_libc():
                warnings.warn(
            ...
            # FIXME (compiler as nodes): recover target compatibility
            ...
            self.compilers.add(c)

    def possible_compilers(self) -> List["spack.spec.Spec"]:
...
class RuntimePropertyRecorder:
    ...
    def requires(self, impose: str, *, when: str):
        ...
        # FIXME (compiler as nodes): check this
    ...
    def default_flags(self, spec: "spack.spec.Spec"):
        # FIXME (compiler as nodes): reduce duplication among the methods in this class
